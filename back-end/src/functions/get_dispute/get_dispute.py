...
def lambda_handler(event, context):
    ...
    # Query item for only PK the disputes from Dynamodb table
    try:
    ...
    LOGGER.info("Found number of disputes: " + str(len(found_item)))
    if len(found_item) == 0:
        LOGGER.warning("No disputes found for id: " + id)
        return {
                'statusCode': 404,
                'body': json.dumps({
                    "message": "not_found",
                    "reason": "No disputes found",
                    "detailed_reason": "No disputes found for id: " + id
            }),
        } 
