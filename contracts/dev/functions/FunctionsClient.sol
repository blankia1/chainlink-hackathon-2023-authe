...
abstract contract FunctionsClient is FunctionsClientInterface {
  ...
  function handleOracleFulfillment(
    ...
    bytes memory err
  ) external override recordChainlinkFulfillment2(requestId) {
   ...
   */
  function addExternalRequest(address oracleAddress, bytes32 requestId) internal notPendingRequest2(requestId) {
   ...
   */
  modifier recordChainlinkFulfillment2(bytes32 requestId) {
   ...
   * @param requestId The request ID for fulfillment
   */
  modifier notPendingRequest2(bytes32 requestId) {
